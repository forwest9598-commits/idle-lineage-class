...
function _allyDamageMob(ally, t, dmg, ele) {
    ...
    t.justHit = ele;
    mobWake(t);
    // 🌑 v3.3.33 血壁空間（傭兵中央傷害匯流點）：ele 明示 'magic'＝魔法反射，其餘依武器遠近判定
    if (typeof reflectWallOnDamage === 'function' && t._reflectWall) {
        let _rwW = ally.eq && ally.eq.wpn && DB.items[ally.eq.wpn.id];
        let _rwKind = (ele === 'magic') ? 'magic' : ((_rwW && (_rwW.isBow || _rwW.ranged)) ? 'ranged' : 'melee');
        reflectWallOnDamage(t, dmg, _rwKind, ally);
    }
